...
$(document).ready(function(){
		...
		//Get the list of script Ids based on defect priority
		function getHighPriorityDefectsId(orgScriptsCsvFile) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					reader.onload = function(event){
						alert('reading');
						/*var orgCsv = event.target.result;
					
						var rows = orgCsv.split("\n");
						alert(rows.length);*/
					}
				},
		...
		//Get the question options value
		...
		$(".exportScriptsIdData").on('click', function(event) {
			...
			if (questionsVal == ''){
				...
				var args = [q3PriorityJobList, 'JobIDs.csv'];
				exportScriptIdsToCSV.apply(this, args);	
